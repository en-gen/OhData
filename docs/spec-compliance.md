...
| Request body validated against the EDM's nullability | §11.4.2 / JSON format | ✅ | #355. A `null` for a structural property the framework's own `$metadata` declares `Nullable="false"` is answered `400` (`code: "InvalidBody"`, `target:` the property) **before the handler runs** — previously it reached the handler and the persistence layer's rejection surfaced as a generic `500`. Applies to `POST`, `PUT`, `PATCH`, the navigation-`POST` create route and the structural-property writes. On `POST`/`PUT` an *omitted* property is a violation too, to the extent it is observable — the check reads the bound instance, so a CLR declaration carrying a non-null initializer (`= ""`) is never null and is never reported. The entity **key** is exempt on every verb (a server-generated key is routinely omitted on create, and every EDM key is `Nullable="false"`). Per-entity-set opt-out: `ValidateRequestBodyNullability = false`. Nullability *inside* a nested complex value is not checked |
