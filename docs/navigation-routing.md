...
- `GET /{Set}({key})/{Nav}/$count` accepts **`$format` and nothing else**, and refuses every other `$`-prefixed option. The handler invokes the navigation delegate and counts what comes back, so it applies no data option whatsoever — not even the `$filter` the entity-set `/$count` applies. It rejected nothing at all before #359. **⚠ Breaking:** `$top` and `$skip` were accepted no-ops here from 1.0.0 through 1.7.x and are `400` now; neither can change a count, and an option the route does not act on is refused rather than ignored. Drop them — they never did anything. See [query-options.md](query-options.md#count).
