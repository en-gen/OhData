...
- **Delegate-less navigation** (a bare `HasMany`/`HasOptional`/`HasRequired`) → **SQL-JOIN
  ...
  no N+1. There is no delegate on this path, so there is **no per-navigation
