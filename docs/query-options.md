...
That rejection is resolved from the navigation's **Model B treatment** (see the `$expand` delegate-safety section), not from which navigation the expansion walker happens to reach, so it does not depend on how the navigation was arrived at (#320):
...
- **The continuation *link* is `GetQueryable`-collection-route-only. The *ceiling* now also applies on
  ...
  (`InvalidQueryOption`) instead of the whole collection. `ExpandPagingEnabled` buys nothing there:
  the link would need page 1 and the continuation to agree on an order, and on this route the
  framework composes neither side — the child rows arrive already materialized inside whatever the
  `GetById` handler returned, while the continuation orders by the child key *in the database*.
  ...
  the collection was loaded by the `GetById` handler before the framework saw it, so this is a data
  ceiling only (the #299 trade). Size the eager loads in your `GetById` accordingly, or do not eager-
  load at all — a `GetById` that does not `Include` the navigation serves `[]` and never trips it.
...
> | Nested `$count` over the ceiling → `400` (#254) | the JSON shaping pass over the pushed-down expand tree | **`GetQueryable` collection route only.** |
> | Bare `$expand` over the ceiling → `400`, or the `Nav@odata.nextLink` continuation when `ExpandPagingEnabled` is on (#313) | same shaping pass | **`GetQueryable` collection route only.** |
> | Any expanded **delegate-less collection** navigation over the ceiling → `400` (#418) | a size check over the serialized entity, after the expand pipeline | **`GET /{Set}({key})` only.** Never a link, whatever `ExpandPagingEnabled` says. |
>
> The reason the second and third are narrower is structural, not an oversight in either change: both are computed *after* the expand pushdown composes its SQL bound, and the pushdown runs only on the `GetQueryable` collection route. `GetAll` and Priority-1 have no pushdown at all, and `GET /{Set}({key})` applies `$expand` through the delegate/batch-handler pipeline instead — which is why the fourth row exists as a separate mechanism rather than as extra reach for the third.
>
> **The consequence worth knowing before you rely on the ceiling as a DoS bound:** the fourth row is a **data** ceiling, not a materialization bound. On `GET /{Set}({key})` the related rows are loaded by your own `GetById` handler (typically an EF `Include`) before the framework ever sees the entity, so the `400` is raised *after* that load — it stops the over-sized collection reaching the client, and stops it being served silently truncated, but it cannot stop the query. The worst case for `GET /{Set}({key})?$expand=Nav` is therefore exactly what your `GetById` eager-loads, unchanged by `MaxExpandTop`. The mitigation is in the handler: do not eager-load an unbounded child collection (a `GetById` that does not `Include` it serves `[]`), or bound the `Include` yourself. `GetAll` and Priority-1 remain outside all four rows.
