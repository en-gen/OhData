...
- **The ceiling and the link are `GetQueryable`-collection-route-only, so `GET /{Set}({key})?$expand=Nav`
  is neither bounded nor linked** ([#418](https://github.com/en-gen/OhData/issues/418)). Both the
  bare-shape `400` and the continuation emission are computed in the JSON shaping pass over the
  pushed-down expand tree, and that pass runs on one route. The single-entity read applies `$expand`
  through the delegate/batch-handler pipeline instead, which composes no SQL bound to check against.
  **Unlike the four limits above this is a gap rather than a decision** — it is listed here because it
  is the one place the ceiling does not reach that a reader would reasonably expect it to. It is
  *pre-existing*: nothing bounded the bare shape on any route before #313, which closed it where the
  pushdown runs. The explicit-nested-`$top` ceiling **does** apply on that route (#301).
...
>
> The reason the second and third are narrower is structural, not an oversight in either change: both are computed *after* the expand pushdown composes its SQL bound, and the pushdown runs only on the `GetQueryable` collection route. `GetAll` and Priority-1 have no pushdown at all, and `GET /{Set}({key})` applies `$expand` through the delegate/batch-handler pipeline instead.
>
> **The consequence worth knowing before you rely on the ceiling as a DoS bound:** `GET /{Set}({key})?$expand=Nav` materializes the whole related collection with `MaxExpandTop` set, and emits neither a `400` nor a link. This is **pre-existing** — nothing bounded the bare shape anywhere before #313, which closed it on the collection route and left the single-entity route where it already was — and it is tracked in [#418](https://github.com/en-gen/OhData/issues/418).
