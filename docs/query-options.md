...
>
> **`$search` is `501` on `/$count` (`GET /odata/Products/$count?$search=alpha`).** Ignoring it
> would return the **unfiltered** total under a `200`, which §11.2.9 forbids and which no client
...
>
> **The four §11.2.9 names, and `$select`, are accepted and ignored.** Ignoring them is what the
> clause specifies, so under Minimal item 7's *"either follow the specification or return 501 - for
> any unsupported functionality"* it is the follow arm; a `501` there would claim non-implementation
> of something this route does correctly. This also matches `Microsoft.AspNetCore.OData`, whose
