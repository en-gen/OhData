...
Pushdown is **on by default** (`EntitySetDefaults.ExpandPushdownEnabled`, per-profile `ExpandPushdownEnabled` override). It engages **only** on the EF Core-backed `GetQueryable` path and **only** for a navigation whose related type has no back-reference cycle. Whenever pushdown is *structurally* ineligible for a request — a non-EF provider, a cyclic navigation, a deferred nested option (see the table below), or a projection that cannot even be constructed (e.g. a complex or unsettable structural member) — it **falls back silently**: the delegate-less navigation simply stays EDM-only for that request (as it was before this feature), the request still succeeds (never a `500`), and the reason is `Debug`-logged. **"Stays EDM-only" means the framework doesn't itself load the navigation — it does not mean the navigation is guaranteed to come back empty.** No pushdown means no JOIN and no nulling either: whatever the handler's `GetQueryable`/`GetAll` query already produced for that navigation (a non-EF `IQueryable`'s own eager load, an EF `Include` the handler wrote itself, or a hand-built object graph) passes through untouched and serializes as-is, since `$expand`'d navigations are never stripped by `OmitUnexpandedNavigations` regardless of how they got their data. Only a handler that left the navigation genuinely unpopulated will actually report it empty.

**A translation failure is different, and fails loud (FAIL LOUD, post-#298/#300 review).** If a nested `$filter`/`$orderby` cannot be bound, or the composed query cannot be translated by the provider even though everything looked eligible, the request now returns `400` (`InvalidQueryOption`) instead of silently degrading to EDM-only under a `200` — before this fix, a translation failure could mean the affected navigation (or, for the specific shapes #298/#300 identified, the whole parent collection) came back wrong or empty with no indication anything failed. Simplify the nested option combination, or write an expand **delegate** for that navigation to take full control of its query shape. (One narrower case stays a silent fallback: the folded graph tripping a JSON *serialization* cycle the static back-reference guard missed — by that point the query itself already succeeded, so the safety net re-fetches without pushdown rather than surfacing an error for a shape the static check should have caught.)
...
One caveat: a nested `$top` (not `$skip`) on a self-referential navigation is rejected by the underlying OData validator before OhData's pushdown code ever runs (the navigation's target type is necessarily its own entity set — that is what makes `$levels` legal on it at all — so its model-bound `MaxTop` always defaults to `0`) — see [#296](https://github.com/en-gen/OhData/issues/296). That is a pre-existing limitation independent of `$levels` and reproduces identically on a plain (non-`$levels`) `$expand=Nav($top=…)` against a self-referential navigation, so `$levels` + `$top` is unreachable today. `$skip` carries no such model-bound ceiling and reaches OhData's code: it is applied — like `$count` — in the JSON pass rather than pushed to SQL, because a `$levels` level both windows a collection and projects a further self-referential collection out of it, the same `APPLY`/`LATERAL`-shaped translation problem the `$count` caveat below describes. `?$expand=Children($levels=2;$orderby=name desc;$skip=1)` windows deterministically at every level; the other options are unaffected.
...
- **`$count` on a pushed expand materializes the full filtered child collection; whether `MaxExpandTop` bounds that materialization in SQL depends on the shape.** To report `Nav@odata.count` accurately, the whole filtered set is loaded before `$top`/`$skip` paging is applied — the same amount of data a bare `$expand=Nav` already loads. `$top`/`$skip` *without* `$count` push the paging into SQL and transfer only the page. As of #254, at a **projection leaf** — a level with no nested `$expand` of its own — that materialization **is bounded in SQL**: the framework composes a `Take(MaxExpandTop + 1)`, and a related collection larger than the ceiling is rejected with `400` (`InvalidQueryOption`) rather than reported with a truncated count — §11.2.4.2 requires `Nav@odata.count` to be the count of the **full filtered** collection, so silent truncation would be a lie. At a level that **also** carries its own nested `$expand` (a level with children), or anywhere inside a `$levels` recursion, the SQL bound is **not** composed — windowing a collection *and* projecting a further collection out of it in the same query requires SQL `APPLY`/`LATERAL`, which not every provider (SQLite among them) translates — so the ceiling is instead enforced **after an unbounded materialization**: the full related collection is loaded, then rejected with the same `400` if it exceeds `MaxExpandTop`. The *correctness* of the ceiling is enforced either way (never a truncated count); [#299](https://github.com/en-gen/OhData/issues/299) tracks tightening the unbounded-materialize-then-`400` *cost* of the second case, which stays open. Narrow the collection with a nested `$filter`, or raise/remove `MaxExpandTop`.
