...
A plain (non-`$levels`) `$expand=Children($top=…)` against a **delegate-less self-referential** navigation is a separate, still-open, orthogonal limitation: the plain member-init projection for a self-reference is genuinely cyclic, so it never engages pushdown regardless of `$top` — that request now returns `200` (the old validator `400` is gone) but the `$top` isn't actually applied; only `$levels` bounds the cycle for this shape (see `LevelsWithOptionsPushdownSqliteTests.NestedTop_OnSelfReferentialNav_IsHonored_WithAndWithoutLevels`). `$skip` never carried a model-bound ceiling and always reached OhData's code even before this fix. `?$expand=Children($levels=2;$orderby=name desc;$skip=1)` windows deterministically at every level; the other options are unaffected.
