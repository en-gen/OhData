...
upper bound rather than as a measurement of exception mapping itself: the MS OData control, whose code is
...
- **Identical dataset:** 1000 deterministic `BenchWidget` entities (id, name, category, price,
  ...
  a separate, also-deterministic `BenchDepartment` (20 rows) / `BenchEmployee` (1000 rows) fixture
  generated by `BenchOrgData` and seeded into each host's own EF Core SQLite database. Department
  fan-out is **Zipf-skewed, not uniform** (`DepartmentSkewExponent = 1.3`, seed-shuffled ranks so
  which department ends up the outlier varies with the seed) — deliberately, since a uniform split
  hid the regime where nested-`$top` windowing strategies diverge from "materialize everything and
  count." For the committed default seed (42) that skew produces a min of 8, a max of 387 and a
  median of 18 employees across the 20 departments. The manager tree's branching factor is likewise
  seed-derived, drawn once per seed from `[MinManagerBranchingFactor=3, MaxManagerBranchingFactor=7]`
  rather than fixed.
...
6. **`@odata.nextLink` on the `BenchDepartments` page.** OhData emits one; MS OData does not. Every
   department-rooted URL sends an explicit `$top=12` (`BenchOrgData.DepartmentPageSize`, kept below
   `DepartmentCount`'s 20 so root paging is genuinely exercised — see asymmetry 8), so both hosts
   return the same 12-row page. OhData treats `MaxTop` as an implicit page size and advertises a
   next page whenever the returned page is full — correctly here, since 8 departments remain. MS
   OData's `[EnableQuery(MaxTop=...)]` only caps a client-*supplied* `$top`; with no `PageSize` set
   on the controller (asymmetry 9) it adds no continuation semantics of its own, so it never emits a
   link even though more data exists. Row content is identical either way — no timing distortion —
...
8. **`MaxTop` means different things on the two hosts.** OhData treats it as an implicit page
   ...
   caps a client-*supplied* `$top` and does nothing to an unpaged request on its own.
   `DepartmentPageSize` (12) is deliberately **below** `DepartmentCount` (20) so root paging is
   genuinely exercised (see asymmetry 6), which would otherwise let this divergence show up as a
   row-count mismatch: every department-rooted URL in this suite therefore sends an explicit
   `$top=12` (`BenchmarkRequests`), so both hosts window to the identical 12 rows regardless of
   which `MaxTop` behaviour is in play. The one `BenchEmployees` scenario (`$levels`) sidesteps it a
   different way, by `$filter`ing down to a single row before `$expand`ing. Neither safeguard is
   structural: an unfiltered, unpaged `BenchEmployees` or department scenario that omitted its
   explicit `$top` would diverge sharply (OhData capped at its `MaxTop`, MS OData returning
   everything), and nothing in this suite would catch it.
