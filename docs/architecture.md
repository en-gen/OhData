...
AddOhData(builder => builder.AddProfile<T>())
  │
  └─► OhDataBuilder collects profile types, stores prefix
        │
        └─► .Register() adds a singleton factory to DI:
              │
...
```
OhData.Abstractions (netstandard2.1)
...
OhData.AspNetCore (net8.0)
  ...
  └─ Microsoft.AspNetCore.OData [8.*, 9)  ← for EDM/CSDL writing

OhData.Abstractions.AspNetCore.OData (net6/5/3.1)
  └─ OhData.Abstractions
  └─ Microsoft.AspNetCore.OData [8.*, 9)  ← ODataEntitySetProfile with ODataQueryOptions<T>
  [NOTE: not yet integrated into endpoint factory — awaiting OData 9.4+ upgrade]
```
