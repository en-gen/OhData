...
internal class ObsRichProfile : EntitySetProfile<int, ObsNode>
{
    ...
    public ObsRichProfile() : base(x => x.Id)
    {
        ...
        CountEnabled = true;
        GetAll = (ct) => OhDataResult.Success<IEnumerable<ObsNode>>(_store);
        GetById = (id, ct) => OhDataResult.Success(_store.FirstOrDefault(n => n.Id == id));
        Post = (n, ct) => { n.Id = 99; _store.Add(n); return OhDataResult.Success<ObsNode>(n); };
        Put = (id, n, ct) => { n.Id = id; return OhDataResult.Success(n); };
        Patch = (id, delta, ct) =>
        {
            var n = _store.FirstOrDefault(x => x.Id == id);
            if (n is not null) delta.Patch(n);
            return OhDataResult.Success(n);
        };
