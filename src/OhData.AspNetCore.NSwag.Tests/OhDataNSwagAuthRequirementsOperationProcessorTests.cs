...
/// <summary>
/// Verifies <see cref="OhDataNSwagAuthRequirementsOperationProcessor"/> (#220): when opted in, each
/// secured operation's description gains a human-readable requirements section drawn from OhData's
/// structured auth data. Proves it is off by default, that exact claim VALUES are withheld at the
/// Kinds disclosure level, that named policies stay opaque, and that resource-only (Layer B) rules
/// render nothing.
/// </summary>
public sealed class OhDataNSwagAuthRequirementsOperationProcessorTests
{
