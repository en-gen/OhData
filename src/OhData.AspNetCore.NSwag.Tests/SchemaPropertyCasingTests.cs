...
/// independent of the host's <c>HttpJsonOptions</c>), the NSwag-generated schema property names
/// must follow the same policy — otherwise the document advertises casing the wire never uses.
/// These tests assert the schema casing exactly (case-sensitive), so they fail if the processor
...
public sealed class SchemaPropertyCasingTests
{
    ...
    {
        // NSwag emits the base class (OrderBase) as its own `allOf` component; it keeps host casing
        // unless the casing map walks base types (#260).
        using JsonDocument doc = await BuildAndFetchAsync(configurePolicy: null);
    ...
    private static bool HasExactProperty(JsonElement resolvedSchema, string propertyName)
    {
        ...
        // NSwag models an inherited type as `allOf: [{$ref base}, {properties: own props}]`, so a
        // derived type's own keys live on an allOf member rather than top-level `properties`.
