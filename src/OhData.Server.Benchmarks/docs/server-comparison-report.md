...
> **Suite has grown since this page was last generated.** The results tables above and the raw
> BenchmarkDotNet capture below are from the original **11-scenario, 22-benchmark** `BenchWidget`
> suite only (`ServerComparisonBenchmarks`) and are unchanged. The suite now also has a **second,
> EF Core/Sqlite-backed half** — 5 more scenarios (`ExpandComparisonBenchmarks`, 10 more
> benchmarks) covering `$expand`/`$levels` — for **16 scenarios / 32 benchmarks total**. Those
> five scenarios' numbers are deliberately **not** included in the tables above: an adversarial
> fairness review found the shared run config too noisy on them to publish trustworthy
> magnitudes (bimodal iteration-time distributions, >1.5× run-to-run swings on unchanged code).
> They now run under their own, heavier config (see `ExpandComparisonBenchmarks`) for
> reproducibility; the owner will decide on republishing this page once numbers are stable across
> repeated runs.
...
- **Two in-process TestServer hosts**, one per framework, measured over the **full HTTP
  ...
  network — the comparison isolates the HTTP/OData pipeline itself. The original 11 `BenchWidget`
  scenarios also have no database (`List<T>`-backed store); the 5 newer `$expand`/`$levels`
  scenarios are backed by EF Core + Sqlite (an in-memory keep-alive connection, one independently
  seeded per host) because OhData's `$expand` pushdown is gated to an EF Core-backed `IQueryable`
  — see `BenchmarkHosts` and `Model/BenchOrgData.cs`.
  - **OhData host:** `AddOhData` + `MapOhData` minimal-API endpoints over
    `EntitySetProfile<int, BenchWidget>` (`GetQueryable` path) for the original 11 scenarios, and
...
- **Identical dataset:** 1000 deterministic `BenchWidget` entities (id, name, category, price,
  ...
  The `$expand`/`$levels` scenarios use a separate, also-deterministic `BenchDepartment`
  (20 rows)/`BenchEmployee` (1000 rows, uniform 50-per-department fan-out, a branching-factor-5
  manager tree) fixture generated by `BenchOrgData` and seeded into each host's own EF Core Sqlite
  database — see `BenchmarkRequests.cs` for the specific requests and the known asymmetries they
  carry.
...
- **Run config:** the original 11 `BenchWidget` scenarios keep
  `[SimpleJob(warmupCount: 5, iterationCount: 20)]` + `[MemoryDiagnoser]` — iteration counts
  trimmed from BenchmarkDotNet's adaptive default so that half of the suite completes quickly
  while keeping error bars small relative to the inter-server deltas reported. The 5
  `$expand`/`$levels` scenarios run under a separate, heavier config (10 warmup / 30 measured
  iterations, explicit `MinIterationTime`) chosen specifically to fix the noise the adversarial
  review found — see `ExpandComparisonBenchmarks` for the rationale.
