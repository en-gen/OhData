...
**Open types are EDM-driven, complex-only, and attribute-free (#389).** `OpenTypeJsonOptions` reads `ODataConventionModelBuilder`'s own `DynamicPropertyDictionaryAnnotation` (via `EdmAnnotationExtensions.GetDynamicPropertyDictionary`) for every complex type the EDM marks `IsOpen`, and layers one more `TypeInfoResolver` modifier onto the registration's `effectiveJsonOptions` in `MapAll` that sets `JsonPropertyInfo.IsExtensionData = true` on exactly that member — so the bag serializes/binds **flat** with no `[JsonExtensionData]` on the consumer's model. Ordering is load-bearing: **after** `IgnoredPropertyJsonOptions.Build` (so `Ignore()` on the container still wins) and **before** the per-request nav-suppression modifier (which derives from these options, so `WithAddedModifier` chains all three). The member is matched by `MemberInfo.HasSameMetadataDefinitionAs`, **not** `==`/`ReferenceEquals` — the builder's annotation and STJ's `AttributeProvider` come from independent reflection walks that disagree on `ReflectedType` (the builder can report a *derived* type there), so `PropertyInfo` equality returns false for the same member. Entity-root containers and `$filter`/`$orderby` over individual dynamic keys are deliberately out of scope; `docs/open-types.md` and `OpenTypeLimitationTests` record both.
